...
const CreateOms  = ({ onClose, Responsables, LastOmc, Tipe, Componentes, Mantenimientos }) =>  {
  ...
  const formik = useFormik({
    initialValues:initialValue(LastOmc, Tipe),
        ...
        <div className='w-1/2 h-auto flex-col justify-center items-center gap-3'>
          ...
          <select 
            ...
            onChange={(e) => {
              ...
              const categoriaId = Componentes.find(
            ...
            {
              Componentes ? (
                Componentes.filter(data => data.nombre.includes(filtroComponente)).map((data) => (
                  <option 
