...
import { initialValue, validationSchema } from './FormEditActividadMtto.form';
import { useForm } from '@inertiajs/react'

const EditActMtto = ({ onClose, Actividad }) =>  {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) => {
      ...
      data.tipofrecuencia = Actividad.tipofrecuencia
      data.taqManto       = Actividad.taqManto
      post('/actividades/mantenimiento/update')
      onClose();
    }
  })
  ...
  return (
    <form 
      ...
      >
        ...
        <input type="submit" value = "Actualizar Actividad" className="w-full h-auto px-4 py-2 border border-green-500 hover:border-white bg-green-500 hover:bg-green-800 text-white  font-bold  rounded-sm cursor-pointer transition duration-700 ease-in-out" />
    </form>
  )
}
