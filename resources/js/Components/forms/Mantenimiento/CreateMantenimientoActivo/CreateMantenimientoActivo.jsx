...
import { initialValue, validationSchema } from './CreateMantenimientoActivo.form';
import { useForm } from '@inertiajs/react'
import { useState } from "react";

const CreateMantenimientoActivo = ({ onClose, CategoriasActivos }) =>  {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) => {
        ...
        post('/mantenimiento/activo/store')
        onClose()
    }
  })
  return (
    ...
    >
        <h3 className='font-bold'>
          REGISTRANDO NUEVO MANTENIMIENTO DE ACTIVO
        </h3>
        <div className='w-full h-auto flex-col justify-center items-center gap-3'>
            ...
            {
                CategoriasActivos ? (
                    CategoriasActivos.filter(data => data.nombre.includes(filtro)).map((data) => (
                        <option 
...
export default CreateMantenimientoActivo;
