...
import { initialValue, validationSchema } from './FormNewEmpresa.form';
import { useForm } from '@inertiajs/react'


const NewEmpresa = ({ onClose, status }) =>  {

  useEffect(() => {
    if(status === 'Empresa Registrada Correctamente'){
      onClose()
    }
  }, [status]);
  ...
  return (
    <form 
      ...
      >
        ...
        <input type="submit" value = "Crear Empresa " className="w-full h-auto px-4 py-2 border border-green-500 hover:border-white bg-green-500 hover:bg-green-800 text-white  font-bold  rounded-sm cursor-pointer transition duration-700 ease-in-out" />
