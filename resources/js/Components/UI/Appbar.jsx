...
const Appbar = ({ Objeto, ShowImage, ShowActions, ShowFormats, Taq, urlImage }) => {    
  return (
    <div className='w-full h-[10%] sm:h-auto px-8 py-4 bg-[#385449] flex flex-col sm:flex-row justify-start items-center justify-items-center gap-6'>
      <div className='w-full h-full sm:h-[150px] sm:px-8 md:py-4 bg-[#385449] flex justify-start items-center justify-items-center gap-6'>
        <div className=' w-full sm:w-[60%] h-auto text-white font-bold flex flex-col justify-center items-start smitems-center sm:items-start justify-items-center order-2'>
          {
            Objeto.nombre.length > 0 ? <h3 className=' text-sm sm:text-base md:text-lg'> { Objeto.nombre } </h3> : null
          }
          {
            Taq != 'not_show' ? (
              <>
                {
                  Taq.length > 0 ? <h3 className='hidden sm:flex md:text-lg '> { Taq } </h3> : null
                }
              </>
            ) : null
          }
          {
            Objeto.serial ? <h3 className='hidden sm:flex md:text-lg '> { Objeto.serial } </h3> : null
          }
        </div>
        {
          urlImage != 'not_image' ? (
            <div onClick = { ShowImage } className='flex min-w-[150px] w-auto   h-full z-10  rounded-md overflow-hidden border-2 border-white order-1 cursor-pointer hover:scale-105 transition duration-700 ease-in-out'>
              {
                Objeto.urlImage != 'default-image.jpg' ? (
                  <img src={urlImage} alt={`image: ${Objeto.nombre}`} className = ' w-full h-full object-cover sm:object-fill ' loading="lazy"/>
                ) : (
                  <img src={`https://gworks.gematech.co/storage/${Objeto.urlImage}`} alt={`image: ${Objeto.nombre}`} className = ' w-full h-full object-fill ' loading="lazy"/>
                )
              }
            </div>
          ) : null
