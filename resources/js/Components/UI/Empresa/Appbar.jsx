...
const EmpresaAppbar = ({ nombre, link, Empresa, Resposanble, CategoriasActivo, CategoriasHerramientas, Cargos }) => {
  ...
  return (
    <div className='w-full h-[10%] sm:h-auto px-8 py-4 bg-[#385449] flex justify-start items-center justify-items-center gap-6'>
        <div className='w-full h-full sm:h-[150px] sm:px-8 md:py-4 bg-[#385449] flex justify-start items-center justify-items-center gap-6'>
          ...
          <div className='hidden sm:flex w-auto h-full z-10 bg-white p-4  rounded-md overflow-hidden border-2 border-white order-1'>
            <img src={link} alt="" className = ' w-full h-full object-fill ' loading="lazy"/>
          </div>
        </div>
        ...
        <Modal
          ...
          tittle    = {`Acciones`}
        >
          <Actions Cargos = { Cargos } Empresa = { Empresa } Resposanble = { Resposanble } onClose = { () => setModalVisible(false) } CategoriasActivo = { CategoriasActivo } CategoriasHerramientas = { CategoriasHerramientas }/>
        </Modal>
    </div>
  )
}

export default EmpresaAppbar;